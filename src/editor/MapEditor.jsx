...
const DEFAULT_PRESET_INDEX = 1; // Srednja
...
export default function MapEditor() {
    ...
    useEffect(() => {
        ...
        // postavi mapu na default preset
        const defaultPreset = MAP_PRESETS[DEFAULT_PRESET_INDEX];
        sm.setMapSize(defaultPreset.gridSize);

        tm.loadAll().then(() => {
            const terrain = new TerrainMesh(sm.scene);
            terrain.init(tm.get('grass1'), sm.renderer, defaultPreset.paintRes);
    ...
    function handleSelectTexture(id) {
        ...
        const tex = textureManagerRef.current?.get(id);
        if (tex && terrainRef.current?.material) {
            terrainRef.current.material.uniforms.uGrassTex.value = tex;
        }
    }
